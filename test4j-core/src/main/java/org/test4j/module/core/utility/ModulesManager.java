...
public class ModulesManager {
    ...
    public static <T extends Module> T getModuleInstance(Class<T> type) {
        ...
        if (modulesOfType.size() > 1) {
            throw new Test4JException("More than one module found of type " + type.getName());
        }
        if (modulesOfType.size() < 1) {
            throw new Test4JException("No module found of type " + type.getName());
        }
    ...
    public static boolean isModuleEnabled(Class<? extends Module> moduleClass) {
        ...
        if (modulesOfType.size() > 1) {
            throw new Test4JException("More than one module found of type " + moduleClass.getName());
        }
