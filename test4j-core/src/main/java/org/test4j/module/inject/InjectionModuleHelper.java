...
public class InjectionModuleHelper {
     ...
     * @param property       The OGNL expression that defines where the object will be
     *                       injected, not null
    ...
    public static Object injectInto(Object objectToInject, Object target, String property) {
        ...
        try {
            FieldAccessor fieldAccessor = FieldAccessor.field(target, property);
     ...
     *
     * @param objectToInject     The object that is injected
     * @param objectToInjectType The type of the object. This should be the type of the object
    ...
    public static void injectIntoAnnotatedFields(Object objectToInject, Object target,
        ...
        for (Field annotatedField : annotatedFields) {
            FieldAccessor.field(annotatedField).set(target, objectToInject);
        }
    }
    ...
    private static Object injectIntoFieldByType(Object objectToInject, Type objectToInjectType, Object target,
        ...
        Set<Field> fieldsWithExactType = Reflector.getFieldsOfType(targetClass, objectToInjectType);
        if (fieldsWithExactType.size() > 1) {
            ...
            Set<Field> fieldsOfType = Reflector.getFieldsAssignableFrom(targetClass, objectToInjectType);
        ...
        FieldAccessor accessor = FieldAccessor.field(fieldToInjectTo);
        Object oldValue = null;
        try {
            oldValue = accessor.get(target);
        } catch (Throwable e) {
            MessageHelper
                    .warn("Unable to retrieve current value of field to inject into. Will not be able to restore value after injection.",
                            e);
        }
        accessor.set(target, objectToInject);
        return oldValue;
    }
}
