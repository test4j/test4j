...
public class ExceptionWrapper {
     ...
     * @param e
     */
    public static RuntimeException wrapAdvisedException(Object object, Exception e) {
        try {
            ...
            boolean isProxy = MethodAccessor.method(advisedClaz, "isProxyTargetClass").invoke(object);
            if (isProxy) {
                sb.append("proxied by the full target class");
                return new RuntimeException(sb.toString(), e);
            } else {
                Class[] clazzes = MethodAccessor.method(advisedClaz, "getProxiedInterfaces").invoke(object);
