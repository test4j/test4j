...
public class ResourceHelperTest implements ICore {
    ...
    private final static String codedir = System.getProperty("user.dir") + "/../test4j.core/src/main/java";

    @Test
    public void findWikiFile_FilePath() throws Exception {
        File file = ResourceHelper.findWikiFile(null,
                String.format("file:%s/org/test4j/tools/commons/ResourceHelper.java", codedir));
        want.file(file).isExists();
    }

    @Test
    public void findWikiFile_ClassPath() throws Exception {
        InputStream stream = ResourceHelper.getResourceAsStream("org/test4j/tools/commons/ResourceHelperTest.class");
        want.object(stream).notNull();
        File file = ResourceHelper.findWikiFile(null, "org/test4j/tools/commons/ResourceHelperTest.class");
        want.file(file).isExists();
    }
    ...
    public void getFileEncodingCharset_utf() {
        File utf8 = new File("src/test/resources/org/test4j/tools/commons/is_utf8.txt");
    ...
    public void getFileEncodingCharset_gbk() {
        File gbk = new File("src/test/resources/org/test4j/tools/commons/is_gbk.txt");
    ...
    public void testGetResourceAsStream_file() throws FileNotFoundException {
        InputStream is = ResourceHelper
                .getResourceAsStream("file:src/test/resources/org/test4j/tools/commons/executeFile.sql");
    ...
    public void testGetResourceAsStream_classpath() throws FileNotFoundException {
        InputStream is = ResourceHelper.getResourceAsStream("classpath:org/test4j/tools/commons/executeFile.sql");
        want.object(is).notNull();
        String sql = ResourceHelper.convertStreamToSQL(is);
        want.string(sql).contains("insert into tdd_user").notContain("--").notContain("#");
    }
    ...
    public void testGetResourceAsStream_classpath2() throws FileNotFoundException {
        InputStream is = ResourceHelper.getResourceAsStream("org/test4j/tools/commons/executeFile.sql");
        want.object(is).notNull();
        String sql = ResourceHelper.convertStreamToSQL(is);
        want.string(sql).contains("insert into tdd_user").notContain("--").notContain("#");
    }
