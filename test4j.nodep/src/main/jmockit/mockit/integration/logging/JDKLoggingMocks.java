...
/**
 * A mock class containing mocks and stubs for the standard logging API in the JDK (java.util.logging).
 * <p/>
 * When a test class is annotated as {@code @UsingMocksAndStubs(JDKLoggingMocks.class)}, all production code touched by
 * the tests in that class will receive mock {@code Logger} instances instead of real ones, when one of the factory
